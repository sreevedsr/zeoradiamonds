<x-app-layout>
        ...
        <div class="grid gap-6 grid-cols-2 xl:grid-cols-4">
            ...
            <div class="p-6 rounded-2xl shadow-md bg-white dark:bg-gray-800 hover:shadow-xl hover:scale-[1.02] transition-all">
                <div class="flex items-center justify-between">
                    ...
                    <div class="w-10 h-10 rounded-xl bg-gradient-to-br {{ $metrics['sales_diff'] >= 0 ? 'from-blue-500 to-indigo-500' : 'from-red-500 to-rose-500' }} flex items-center justify-center shadow">
                        <span class="text-white font-bold text-lg">%</span>
                    </div>
                </div>
                <p class="mt-4 text-3xl font-bold text-gray-900 dark:text-gray-100">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            const labels = @json($metrics['months']);
            const sales = @json($metrics['sales_series']);
            const purchases = @json($metrics['purchase_series']);
