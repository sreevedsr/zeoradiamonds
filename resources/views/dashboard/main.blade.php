<x-app-layout>
    ...
    <div class="grid gap-6 mb-8 md:grid-cols-3">
        <div class="p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
            ...
            <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">{{ ucfirst($scope) }} view</p>
        </div>

        <div class="p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
            <p class="text-sm text-gray-600 dark:text-gray-400">Sales This Month</p>
            <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">
                {{ money_inr($metrics['sales_this_month']) }}</p>
        </div>

        <div class="p-4 bg-white rounded-lg shadow-xs dark:bg-gray-800">
            <p class="text-sm text-gray-600 dark:text-gray-400">New Customers (This Month)</p>
            <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">{{ $metrics['new_customers_this_month'] }}
            </p>
        </div>
    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            const labels = @json(array_column($metrics['monthly_sales'], 'label'));
            const values = @json(array_column($metrics['monthly_sales'], 'value'));
