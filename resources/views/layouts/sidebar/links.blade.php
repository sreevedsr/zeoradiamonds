<ul class="mt-6">
    ...
    @can('edit-cards')
        <li class="relative px-6 py-3" x-data="{
            ...
            <!-- Active bar -->
            <span
                class="{{ request()->routeIs('admin.products.*') ? 'absolute inset-y-0 left-0 w-1 bg-purple-600 rounded-tr-lg rounded-br-lg' : '' }}"
                aria-hidden="true"></span>
    ...
    @can('view-market')
        <li class="relative px-6 py-3" x-data="{
            openMarket: {{ request()->routeIs('merchant.marketplace.*') ? 'true' : 'false' }},
            height: 0,
            setMeasured() {
                this.height = this.$refs.panel ? this.$refs.panel.scrollHeight : 0;
            },
