...
<div x-data x-show="$store.purchaseModal.show" x-transition.opacity.duration.200ms
    ...
    <div x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95 translate-y-3"
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h2 id="sale-modal-title" class="text-lg font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2">
        ...
        <form id="saleItemForm" x-ref="saleForm" x-data="saleItemForm()"
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div>
                    ...
                    <input type="text" x-model="barcode" placeholder="Scan or type barcode" class="input-field w-full rounded-md border px-3 py-2 text-sm">
                </div>

                <!-- Product Code -->
                <div>
                    <label class="text-sm font-medium">Product Code</label>
                    <input type="text" x-model="product_code" placeholder="Product Code" class="input-field w-full rounded-md border px-3 py-2 text-sm">
                </div>

                <!-- Item Code (auto generated/lookup) -->
                <div>
                    <label class="text-sm font-medium">Item Code</label>
                    <input type="text" x-model="item_code" readonly class="w-full rounded-md border px-3 py-2 bg-gray-50 dark:bg-gray-700 text-sm">
                </div>

                <!-- Item Name -->
                <div>
                    <label class="text-sm font-medium">Item Name</label>
                    <input type="text" x-model="item_name" readonly class="w-full rounded-md border px-3 py-2 bg-gray-50 dark:bg-gray-700 text-sm">
                </div>

                <!-- HSN Code -->
                <div>
                    <label class="text-sm font-medium">HSN Code <span class="text-red-500">*</span></label>
                    <input type="text" x-model="hsn" required class="input-field w-full rounded-md border px-3 py-2 text-sm">
                </div>

                <!-- Quantity -->
                <div>
                    <label class="text-sm font-medium">Quantity</label>
                    <input type="number" min="0" step="1" x-model.number="quantity" @input="recomputeAll" class="input-field w-full rounded-md border px-3 py-2 text-sm">
                </div>

                <!-- Gross Weight -->
                <div>
                    <label class="text-sm font-medium">Gross Weight (g)</label>
                    <input type="number" min="0" step="0.001" x-model.number="gross_weight" @input="recomputeAll" class="input-field w-full rounded-md border px-3 py-2 text-sm">
                </div>

                <!-- Stone Weight -->
                <div>
                    <label class="text-sm font-medium">Stone Weight (g)</label>
                    <input type="number" min="0" step="0.001" x-model.number="stone_weight" @input="recomputeAll" class="input-field w-full rounded-md border px-3 py-2 text-sm">
                </div>

                <!-- Diamond Weight -->
                <div>
                    <label class="text-sm font-medium">Diamond Weight (g)</label>
                    <input type="number" min="0" step="0.001" x-model.number="diamond_weight" @input="recomputeAll" class="input-field w-full rounded-md border px-3 py-2 text-sm">
                </div>

                <!-- Net Weight (auto = gross - stone - diamond) -->
                <div>
                    <label class="text-sm font-medium">Net Weight (g)</label>
                    <input type="number" x-model.number="net_weight" readonly class="w-full rounded-md border px-3 py-2 bg-gray-50 dark:bg-gray-700 text-sm">
                </div>

                <!-- Net Amount (including gold rate) -->
                <div>
                    <label class="text-sm font-medium">Net Amount (₹)</label>
                    <input type="number" min="0" step="0.01" x-model.number="net_amount" @input="recomputeTaxes" class="input-field w-full rounded-md border px-3 py-2 text-sm">
                    <div class="text-xs text-gray-500 mt-1">Include gold weight rate / item value here.</div>
                </div>

                <!-- Tax display -->
                <div class="col-span-1 sm:col-span-2 lg:col-span-3 grid grid-cols-1 sm:grid-cols-3 gap-3 mt-2">
                ...
                <div class="col-span-1 sm:col-span-1 lg:col-span-3">
                    ...
                    <input type="text" x-model="total_amount_display" readonly class="w-full rounded-md border px-3 py-2 bg-gray-50 dark:bg-gray-700 text-sm">
                </div>
            </div>
...
<script>
    function saleItemForm() {
        return {
            ...
            merchant_state: '',
            company_state: <?php echo json_encode($companyState ?? config('app.company_state') ?? '', 15, 512) ?>,
            ...
            recomputeAll() {
                ...
                const net = (parseFloat(this.gross_weight) || 0) - (parseFloat(this.stone_weight) || 0) - (parseFloat(this.diamond_weight) || 0);
            ...
            recomputeTaxes() {
                ...
                // Determine whether intra-state (CGST+SGST) or inter-state (IGST)
                const intraState = (this.merchant_state && this.company_state) ? (this.merchant_state.trim().toLowerCase() === this.company_state.trim().toLowerCase()) : true;
            ...
            addItem() {
                ...
                if (Alpine.store('purchaseModal') && Array.isArray(Alpine.store('purchaseModal').items)) {
                    ...
                    Alpine.store('purchaseModal', { show: false, items: [item], open() { this.show = true }, close() { this.show = false } });
                }
